...
请你作为系统详细设计专家，根据《系统需求.md》和《大纲.md》编写“03.2 数据处理模块”详细设计。
...
本模块只覆盖自动处理和手动处理两类功能：自动控制硬件进行数据处理并记录处理过程；生成、读取、导出数据库表；基于数据库表比对；引导其他功能；对异常数据进行告警；对数据进行显示；手动控制硬件进行数据处理；数据拼接；自动解析文件名称；调用指定算法；自动生成 Excel 表；把数据保存至文件。

## 子模块拆分要求

- 自动处理子模块：自动控制硬件、处理过程记录、数据库表生成读取导出、数据库表比对、功能引导、异常数据告警。
- 手动处理子模块：数据显示、手动控制硬件、数据拼接、文件名解析、指定算法调用、Excel 生成、文件保存。
...
```plantuml
@startuml
title 数据处理模块子模块操作时序图
actor 操作人员
participant "系统界面" as UI
participant "数据处理模块服务" as Service
...
public interface DataProcessingService {
    OperationResult handle(DataProcessRequest request);
}

public class DataProcessingServiceImpl implements DataProcessingService {
    private final DataProcessRepository repository;

    public DataProcessingServiceImpl(DataProcessRepository repository) {
        this.repository = repository;
    }

    @Override
    public OperationResult handle(DataProcessRequest request) {
    ...
    private OperationResult executeCore(DataProcessRequest request) {
...
- 自动处理必须体现硬件控制与处理过程记录，不扩展为无人值守调度平台。
- 手动处理必须体现人工触发、数据展示、算法选择、Excel 生成和文件保存。
- 数据库表比对只基于数据库表，不扩展为复杂数据治理平台。
- 异常数据告警必须说明告警触发条件、界面提示和记录方式。
