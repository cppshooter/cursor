...
请你作为系统详细设计专家，根据《系统需求.md》和《大纲.md》编写“03.3 硬件交互模块”详细设计。
...
本模块只覆盖硬件交互相关功能：能够控制电源开机；能够接收电源状态信息；能够对硬件返回的结果保存为文件；能够离线处理文件；能够对硬件交互过程进行入库；能够对结果进行入库。

## 子模块拆分要求

- 电源控制子模块：控制电源开机、接收电源状态信息。
- 硬件结果保存子模块：接收硬件返回结果并保存为文件。
- 离线文件处理子模块：对硬件相关文件进行离线处理。
- 交互与结果入库子模块：硬件交互过程入库、结果入库。
...
```plantuml
@startuml
title 硬件交互模块子模块操作时序图
actor 操作人员
participant "系统界面" as UI
participant "硬件交互模块服务" as Service
...
```java
public interface HardwareInteractionService {
    OperationResult handle(HardwareCommandRequest request);
}

public class HardwareInteractionServiceImpl implements HardwareInteractionService {
    private final HardwareInteractionRepository repository;

    public HardwareInteractionServiceImpl(HardwareInteractionRepository repository) {
        this.repository = repository;
    }

    @Override
    public OperationResult handle(HardwareCommandRequest request) {
    ...
    private OperationResult executeCore(HardwareCommandRequest request) {
...
actor 操作人员
actor "其他软件" as External
rectangle "硬件交互模块" {
  usecase "提交业务请求" as UC1
  usecase "校验输入信息" as UC2
  usecase "保存处理记录" as UC3
  usecase "展示处理结果" as UC4
}
操作人员 --> UC1
External --> UC1
UC1 --> UC2
...
<section class="module-panel">
  ...
  <form>
    ...
    <label>处理参数：<input type="text" name="parameter" /></label>
  ...
  <table>
    ...
    <tbody>
      <tr><td>1</td><td>示例记录</td><td>成功</td><td>系统时间</td></tr>
    </tbody>
  </table>
...
- 时序图必须体现“系统界面、硬件交互服务、硬件设备、数据库”的交互。
- 电源控制只要求开机控制，不扩展关机、重启或复杂电源管理。
- 文件处理必须区分硬件返回结果保存为文件与离线处理文件。
- 入库内容必须覆盖交互过程和结果两类记录。
