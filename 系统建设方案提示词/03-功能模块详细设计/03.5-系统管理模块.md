...
请你作为系统详细设计专家，根据《系统需求.md》和《大纲.md》编写“03.5 系统管理模块”详细设计。
...
本模块只覆盖状态监控、策略管理和信息管理三类功能：具备查询多种硬件工作状态的功能；具备硬件自检的功能；具备根据策略生成控制参数的功能；具备增加、修改、删除策略库表内容的功能；具备策略导出功能；具备硬件配置信息管理功能。

## 子模块拆分要求

- 状态监控子模块：查询多种硬件工作状态、硬件自检。
- 策略管理子模块：根据策略生成控制参数、策略库表新增、修改、删除、策略导出。
- 信息管理子模块：硬件配置信息管理。
...
```plantuml
@startuml
title 系统管理模块子模块操作时序图
actor 操作人员
participant "系统界面" as UI
participant "系统管理模块服务" as Service
...
public interface SystemManagementService {
    OperationResult handle(SystemManagementRequest request);
}
...
public class SystemManagementServiceImpl implements SystemManagementService {
    ...
    public SystemManagementServiceImpl(SystemManagementRepository repository) {
        this.repository = repository;
    }

    @Override
    public OperationResult handle(SystemManagementRequest request) {
    ...
    private OperationResult executeCore(SystemManagementRequest request) {
...
- 状态监控必须围绕硬件工作状态和硬件自检，不扩展为综合运维平台。
- 策略管理必须体现策略生成控制参数、策略库表增删改和导出。
- 信息管理必须限定为硬件配置信息管理。
- 界面设计需要区分状态监控、策略维护、配置维护三个区域。
