...
- **Definition of Done:**
  ...
  - 通过 `.claude/knowledge/bid-type.md` 的「强制完整性 / 撰写完整性自检清单」（按本类型逐项核对，缺项即失分）
  - 类型专属产物齐全（软件：总体架构四图 + 逐模块五要素 + 数据库/接口设计；硬件：硬件总体/详细设计 + 偏离表 + 可靠性/EMC + 工艺测试；集成：集成架构+拓扑 + 偏离表 + 互联互通 + 迁移割接联调；服务：服务体系+平台 + 分级流程 + 可量化技术能力指标 + 容灾保障）
