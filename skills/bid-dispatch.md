...
4. compose 阶段合成定稿章节为最终技术方案（唯一直接产出内容的动作）
...
| analysis | analyst | `analysis-order.md` | 技术方案类型、tender/ 路径、需产出的五份技术清单（仅取技术部分） |
| architecture | architect | `architecture-order.md` | 技术方案类型、需覆盖的技术评分点范围 |
| drafting | retriever → writer | `retrieval-order.md` 然后 `write-order.md` | 先写检索 order（目标章节+技术评分点+素材类型），retriever 完成后写撰写 order（目标章节+承接技术评分点+素材路径+重写时的失分项） |
| review | reviewer | `review-order.md` | 审查范围（全文/指定章节） |
| scoring | expert | `scoring-order.md` | 评分范围（仅技术）、score_threshold、当前 rewrite_round |
| compose | （bid-agent 自己）| — | 合成 sections/*.md → output/technical-proposal.md |
...
```
对每个待撰写章节 ch-{N}：
  1. 写 retrieval-order.md（本章技术评分点 + 需要的技术素材类型）→ 调 retriever
  2. 确认 retrieval-result.md 生成、retrieval-order.md 已清理
  3. 写 write-order.md（本章技术评分点 + retrieval-result.md 路径 + 重写时的失分项）→ 调 writer
...
仅当全部技术方案大纲章节都有定稿 `sections/*.md` 且技术评分达阈值时执行：
...
3. order 文件包含子 agent 完成任务所需的完整上下文（含实质性技术否决红线提醒）
4. 不把多个任务塞进同一个 order
...
任何阶段（尤其 review/scoring）发现触碰 `analysis/disqualification.md`（实质性技术否决项）的红线，立即回退 drafting 修正，优先级高于评分优化。

## 范围红线（全流程）

本系统只编写技术方案，调度与产出聚焦评分表-技术部分、实质性要求-技术产品部分、技术任务书/采购需求技术部分。不调度任何商务/报价/资质章节的撰写。
