...
**容量与性能估算。** 数据规模主要由日志/交互/状态三类高增长表决定。按单台试验台日均约 5 万条交互、2 万条状态、1 万条日志估算，三类表年增长约 2900 万行、磁盘量级约 6~10 GB；任务/指令/结果类表年增量在百万行级、磁盘量级 1 GB 以内。控制手段：高增长表统一建立 `(时间, 类型)` 复合索引、界面查询一律分页（默认 200 行/页）、按月归档清理在线库。单机 SQLite 不做分库分表；当在线库超过约 20 GB 或并发写入成为瓶颈时，按 4.1 节兼容设计平滑切换至达梦 DM8 / 人大金仓 KingbaseES，表结构与访问层（Qt SQL）不变。
