...
五层逻辑结构自上而下的依赖关系如下图，上层只依赖相邻下层，不存在跨层与反向依赖。
...
rectangle "界面层\nQMainWindow / QDockWidget / QTabWidget" as L1
rectangle "业务层\n任务管理 / 数据处理 / 硬件交互 / 结果评估 / 系统管理" as L2
rectangle "数据访问层\nQt SQL（QSqlDatabase / QSqlQuery）" as L3
rectangle "硬件交互层\nQSerialPort / QTcpSocket / QTcpServer" as L4
rectangle "存储层\n业务数据库 + 业务文件目录" as L5
...
下图给出核心业务数据从产生到消费的主流向，以及各环节的落库节点。
