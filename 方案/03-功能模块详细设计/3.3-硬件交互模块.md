...
硬件交互是本系统与外部物理设备之间的唯一通路，使用人员在试验前后都要频繁打交道。业务核心是 **"开机—取数—留痕"** 三个连续动作。

1. **谁在使用、什么时候使用**
   - 主要用户：值班操作员（试验前开机、试验中盯状态、试验后断开）、设备工程师（联调、排障）。
   - 触发场景：每天上电前需远程让电源开机并确认电压/温度等参数正常；试验过程中需要持续把硬件返回的数据存盘；试验后或事后离线分析时需要把同一份原始文件再走一遍解析入库；遇到异常需要追溯每一次"发了什么、回了什么"。
2. **当前痛点（如果没有这个模块）**
   - 电源开机依赖手动按键，无法纳入软件流程；
   - 串口 / TCP 两种链路差异大，业务代码与通信细节耦合严重；
   - 硬件返回数据若不及时落盘容易丢失，落盘后又缺乏离线复算能力；
   - 收发报文与最终结果分散在不同位置，无法回溯一次故障的全部"对话"。
3. **本模块的业务价值**
   - **电源远控**：操作员在界面"连接 → 开机 → 心跳"一条龙，背后由 `IPowerController` 抽象层统一屏蔽串口与 TCP 差异，呼应需求"能够控制电源开机；能够接收电源状态信息"。
   - **结果可靠落盘 + 可重放**：在线采集逐段写文件并按上限自动滚动；离线模式可任意加载历史文件再解析，呼应"能够对硬件返回的结果保存为文件；能够离线处理文件"。
   - **交互过程与结果全部入库**：每一次收发报文、每一段解析后的样本块都进入 `t_hw_interaction_log` / `t_hw_result`，并提供检索、清除、重置筛选三类日志动作，呼应"能够对硬件交互过程进行入库；能够对结果进行入库"，同时与系统管理模块的"日志操作 ≥3 种"性能要求对齐。
4. **关键业务流程**：操作员选择硬件 → 建立通信链路 → 下发开机指令 → 周期采集状态与数据 → 在线落盘 + 实时入库 → 试验后加载离线文件再次解析 → 检索 / 导出交互与结果记录用于复核。
