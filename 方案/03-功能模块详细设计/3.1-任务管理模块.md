...
本模块从甲方使用人员日常工作场景出发，回答以下三个问题：**任务是怎么来的、怎么落到本系统里被处理的、做完以后怎么交差**。

1. **谁在使用、什么时候使用**
   - 主要用户：值班操作员（试验台日常值班）与负责人（核查与上报）。
   - 触发场景：上层任务软件在试验前推送本次任务参数；操作员需要立刻看到任务、确认无误、决定何时下发到硬件；试验过程中要随时掌握每条任务跑到了哪一步；试验结束时要把结果和软件本身的技术状态一起报回去。
2. **当前痛点（如果没有这个模块）**
   - 上层下发的任务只能口头或文件转交，容易遗漏；
   - 一次任务往往要分解成数十条硬件指令，靠人工拆分既慢又易错；
   - 试验过程中任务在哪个阶段不透明，出问题难以追溯；
   - 结果与技术状态上报需要重新整理材料，且无统一台账。
3. **本模块的业务价值**
   - **任务统一入口**：所有上层下发的任务在本模块入库留痕，形成"当前任务 / 历史任务"两份可查的视图，呼应需求"能够接收其他软件任务；能够显示当前和历史任务并入库"。
   - **指令自动分解**：按任务类型 + 指令模板表自动生成可执行指令序列，操作员只需复核与下发，呼应"能够分解任务为多种指令"。
   - **过程可视、状态可控**：任务从 `received → decomposed → running → done | failed` 全过程状态可见，呼应"能够对任务状态进行监控和显示"。
   - **结果与技术状态一键回送**：结果报送子模块封装了任务结果与本机软件版本、配置的打包上报与重发，呼应"能够将任务结果上报至其他软件；能够将技术状态上报至其他软件"。
4. **关键业务流程**：上层任务软件 → 任务接收（入库 + 状态显示） → 任务分解为指令序列 → 提交硬件交互模块执行 → 状态变化回写 → 结果与技术状态上报。
