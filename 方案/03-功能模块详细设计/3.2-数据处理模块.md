...
数据处理是甲方使用人员花费时间最多的环节，整体业务可概括为：**白天让机器跑、晚上人手补**。本模块从这个使用习惯切入，将自动化与人工介入两条业务线并行设计。

1. **谁在使用、什么时候使用**
   - 主要用户：值班操作员（启动 / 监控自动处理）、分析员（事后离线分析）。
   - 触发场景：试验过程中需要按既定策略让硬件采集并就地处理；试验结束后对当日批次产物再次复核、拼接、跑算法；遇到异常数据需要立即告警并形成台账。
2. **当前痛点（如果没有这个模块）**
   - 自动采集与数据落盘之间无统一控制，无法保证"采→存→比→评"链路连续；
   - 业务表散落，缺少元数据登记，相互比对靠人工导出再使用第三方工具；
   - 异常数据不被实时识别，容易在事后才发现批次报废；
   - 分析员处理离线文件时没有统一的文件名解析、拼接、算法调用入口，重复工作量大。
3. **本模块的业务价值**
   - **自动处理一条龙**：操作员选定硬件、策略、目标表即可启动批次，过程逐步落入 `t_auto_process_log`；自动生成 / 读取 / 导出业务表、登记到 `t_db_table_meta`；按主键比对两表并落入 `t_table_compare_result`；命中阈值的异常写入 `t_anomaly_alarm` 并即时告警；批次完成后可"引导手动处理 / 引导结果评估"，覆盖需求中的全部自动处理能力。
   - **手动处理工作台**：在同一界面完成数据显示、手动控制硬件、多文件拼接、文件名自动解析、调用算法、自动生成 Excel、保存为文件七件事，每次动作均按 `显示 / 控制 / 拼接 / 解析 / 算法 / 导出 / 保存` 七种取值写入 `t_manual_process_log`，与需求一一对齐，便于事后追溯。
   - **跨模块联动**：批次完成后的 `batch_id` 通过信号同时引导手动处理与结果评估，实现"自动处理→手动复核→结果评估"的业务衔接。
4. **关键业务流程**：操作员配置策略 → 启动自动批次 → 硬件采集、数据落库、过程落日志 → 触发阈值告警 / 表比对 → 批次完成 → 分析员进入手动处理 → 解析、拼接、算法、导出 / 保存 → 结果交付结果评估。
